" Name:         dimlight
" Description:  Light background colorscheme
" Author:       Maxim Kim <habamax@gmail.com>
" Maintainer:   Maxim Kim <habamax@gmail.com>
" License:      Vim License (see `:help license`)
" Last Updated: 04.10.2020 17:12:34

" Generated by Colortemplate v2.1.0

set background=light

hi clear
let g:colors_name = 'dimlight'

let s:t_Co = exists('&t_Co') && !empty(&t_Co) && &t_Co > 1 ? &t_Co : 2

if (has('termguicolors') && &termguicolors) || has('gui_running')
  let g:terminal_ansi_colors = ['#e0e3e3', '#a52a2a', '#2e8b57', '#6a0dad',
        \ '#008787', '#af00af', '#6a5acd', '#bcbcbc', '#808080', '#a52a2a',
        \ '#2e8b57', '#6a0dad', '#008787', '#af00af', '#6a5acd', '#000000']
  if has('nvim')
    let g:terminal_color_0 = '#e0e3e3'
    let g:terminal_color_1 = '#a52a2a'
    let g:terminal_color_2 = '#2e8b57'
    let g:terminal_color_3 = '#6a0dad'
    let g:terminal_color_4 = '#008787'
    let g:terminal_color_5 = '#af00af'
    let g:terminal_color_6 = '#6a5acd'
    let g:terminal_color_7 = '#bcbcbc'
    let g:terminal_color_8 = '#808080'
    let g:terminal_color_9 = '#a52a2a'
    let g:terminal_color_10 = '#2e8b57'
    let g:terminal_color_11 = '#6a0dad'
    let g:terminal_color_12 = '#008787'
    let g:terminal_color_13 = '#af00af'
    let g:terminal_color_14 = '#6a5acd'
    let g:terminal_color_15 = '#000000'
  endif
  if get(g:, 'dimlight_transp_bg', 0) && !has('gui_running')
    hi Normal guifg=#000000 guibg=NONE gui=NONE cterm=NONE
  else
    hi Normal guifg=#000000 guibg=#e0e3e3 gui=NONE cterm=NONE
  endif
  hi EndOfBuffer guifg=#bcbcbc guibg=NONE gui=NONE cterm=NONE
  hi Statusline guifg=#000000 guibg=#bcbcbc gui=NONE cterm=NONE
  hi StatuslineNC guifg=#808080 guibg=#bcbcbc gui=NONE cterm=NONE
  hi StatuslineTerm guifg=#000000 guibg=#bcbcbc gui=NONE cterm=NONE
  hi StatuslineTermNC guifg=#808080 guibg=#bcbcbc gui=NONE cterm=NONE
  hi VertSplit guifg=#bcbcbc guibg=#bcbcbc gui=NONE cterm=NONE
  hi Pmenu guifg=NONE guibg=#bcbcbc gui=NONE cterm=NONE
  hi PmenuSel guifg=#e0e3e3 guibg=#5f5f87 gui=NONE cterm=NONE
  hi PmenuSbar guifg=NONE guibg=#bcbcbc gui=NONE cterm=NONE
  hi PmenuThumb guifg=NONE guibg=#808080 gui=NONE cterm=NONE
  hi TabLine guifg=#808080 guibg=#bcbcbc gui=NONE cterm=NONE
  hi TabLineFill guifg=NONE guibg=#bcbcbc gui=NONE cterm=NONE
  hi TabLineSel guifg=NONE guibg=#e0e3e3 gui=NONE cterm=NONE
  hi ToolbarLine guifg=#e0e3e3 guibg=#d0d0d0 gui=NONE cterm=NONE
  hi ToolbarButton guifg=NONE guibg=#bcbcbc gui=bold cterm=bold
  hi NonText guifg=#bcbcbc guibg=NONE gui=NONE cterm=NONE
  hi SpecialKey guifg=#bcbcbc guibg=NONE gui=NONE cterm=NONE
  hi Folded guifg=#808080 guibg=#d0d0d0 gui=NONE cterm=NONE
  hi Visual guifg=NONE guibg=#ffffd7 gui=NONE cterm=NONE
  hi VisualNOS guifg=NONE guibg=#808080 gui=NONE cterm=NONE
  hi LineNr guifg=#808080 guibg=NONE gui=NONE cterm=NONE
  hi FoldColumn guifg=#808080 guibg=NONE gui=NONE cterm=NONE
  hi CursorLine guifg=NONE guibg=#d0d0d0 gui=NONE cterm=NONE
  hi CursorColumn guifg=NONE guibg=#d0d0d0 gui=NONE cterm=NONE
  hi CursorLineNr guifg=NONE guibg=#d0d0d0 gui=NONE cterm=NONE
  hi QuickFixLine guifg=NONE guibg=#d0d0d0 gui=NONE cterm=NONE
  hi SignColumn guifg=NONE guibg=NONE gui=NONE ctermfg=NONE ctermbg=NONE cterm=NONE
  hi Underlined guifg=#008787 guibg=NONE gui=underline cterm=underline
  hi Error guifg=#d70000 guibg=#e0e3e3 gui=NONE cterm=NONE
  hi ErrorMsg guifg=#d70000 guibg=#e0e3e3 gui=NONE cterm=NONE
  hi WarningMsg guifg=#6a0dad guibg=#e0e3e3 gui=NONE cterm=NONE
  hi MoreMsg guifg=#000000 guibg=#e0e3e3 gui=NONE cterm=NONE
  hi ModeMsg guifg=#000000 guibg=#e0e3e3 gui=NONE cterm=NONE
  hi Question guifg=#2e8b57 guibg=NONE gui=NONE cterm=NONE
  hi Todo guifg=#e0e3e3 guibg=#808080 gui=NONE cterm=NONE
  hi MatchParen guifg=#e0e3e3 guibg=#6a5acd gui=NONE cterm=NONE
  hi Search guifg=#e0e3e3 guibg=#5f5f87 gui=NONE cterm=NONE
  hi IncSearch guifg=#e0e3e3 guibg=#2e8b57 gui=NONE cterm=NONE
  hi WildMenu guifg=#e0e3e3 guibg=#5f5f87 gui=NONE cterm=NONE
  hi ColorColumn guifg=NONE guibg=#d0d0d0 gui=NONE cterm=NONE
  hi Cursor guifg=#e0e3e3 guibg=#000000 gui=NONE cterm=NONE
  hi lCursor guifg=#000000 guibg=#d7005f gui=NONE cterm=NONE
  hi DiffAdd guifg=NONE guibg=#c9f9c9 gui=NONE cterm=NONE
  hi DiffChange guifg=NONE guibg=#f9f9c9 gui=NONE cterm=NONE
  hi DiffText guifg=NONE guibg=#f9d999 gui=NONE cterm=NONE
  hi DiffDelete guifg=#808080 guibg=#f9c9c9 gui=NONE cterm=NONE
  hi SpellBad guifg=#a52a2a guibg=NONE guisp=#a52a2a gui=undercurl cterm=underline
  hi SpellCap guifg=#2e8b57 guibg=NONE guisp=#2e8b57 gui=undercurl cterm=underline
  hi SpellLocal guifg=#6a5acd guibg=NONE guisp=#6a5acd gui=undercurl cterm=underline
  hi SpellRare guifg=#af00af guibg=NONE guisp=#af00af gui=undercurl cterm=underline
  hi Identifier guifg=#008787 guibg=NONE gui=NONE cterm=NONE
  hi Statement guifg=#a52a2a guibg=NONE gui=NONE cterm=NONE
  hi Constant guifg=#d7005f guibg=NONE gui=NONE cterm=NONE
  hi String guifg=#2e8b57 guibg=NONE gui=NONE cterm=NONE
  hi PreProc guifg=#6a0dad guibg=NONE gui=NONE cterm=NONE
  hi Special guifg=#6a5acd guibg=NONE gui=NONE cterm=NONE
  hi Tag guifg=#6a0dad guibg=NONE gui=NONE cterm=NONE
  hi Delimiter guifg=#d7005f guibg=NONE gui=NONE cterm=NONE
  hi Type guifg=#af00af guibg=NONE gui=NONE cterm=NONE
  hi Operator guifg=#6a0dad guibg=NONE gui=NONE cterm=NONE
  hi Directory guifg=#008787 guibg=NONE gui=bold cterm=bold
  hi Comment guifg=#808080 guibg=NONE gui=NONE cterm=NONE
  hi Conceal guifg=#808080 guibg=NONE gui=NONE cterm=NONE
  hi Ignore guifg=NONE guibg=NONE gui=NONE ctermfg=NONE ctermbg=NONE cterm=NONE
  hi Title guifg=#a52a2a guibg=NONE gui=bold cterm=bold
  hi! link colortemplateKey Statement
  hi! link colortemplateAttr String
  hi! link vimNotation Type
  hi! link vimFuncSID PreProc
  hi! link vimHiTerm Identifier
  hi! link helpNotVi Comment
  hi! link helpExample PreProc
  hi! link gdscriptBlockStart Statement
  hi! link gdscriptFunctionName Function
  hi! link gitCommitSummary Title
  hi! link cocErrorSign Type
  unlet s:t_Co
  finish
endif

if s:t_Co >= 256
  if get(g:, 'dimlight_transp_bg', 0)
    hi Normal ctermfg=16 ctermbg=NONE cterm=NONE
  else
    hi Normal ctermfg=16 ctermbg=254 cterm=NONE
  endif
  hi EndOfBuffer ctermfg=251 ctermbg=NONE cterm=NONE
  hi Statusline ctermfg=16 ctermbg=251 cterm=NONE
  hi StatuslineNC ctermfg=244 ctermbg=251 cterm=NONE
  hi StatuslineTerm ctermfg=16 ctermbg=251 cterm=NONE
  hi StatuslineTermNC ctermfg=244 ctermbg=251 cterm=NONE
  hi VertSplit ctermfg=251 ctermbg=251 cterm=NONE
  hi Pmenu ctermfg=NONE ctermbg=251 cterm=NONE
  hi PmenuSel ctermfg=254 ctermbg=60 cterm=NONE
  hi PmenuSbar ctermfg=NONE ctermbg=251 cterm=NONE
  hi PmenuThumb ctermfg=NONE ctermbg=244 cterm=NONE
  hi TabLine ctermfg=244 ctermbg=251 cterm=NONE
  hi TabLineFill ctermfg=NONE ctermbg=251 cterm=NONE
  hi TabLineSel ctermfg=NONE ctermbg=254 cterm=NONE
  hi ToolbarLine ctermfg=254 ctermbg=252 cterm=NONE
  hi ToolbarButton ctermfg=NONE ctermbg=251 cterm=bold
  hi NonText ctermfg=251 ctermbg=NONE cterm=NONE
  hi SpecialKey ctermfg=251 ctermbg=NONE cterm=NONE
  hi Folded ctermfg=244 ctermbg=252 cterm=NONE
  hi Visual ctermfg=NONE ctermbg=230 cterm=NONE
  hi VisualNOS ctermfg=NONE ctermbg=244 cterm=NONE
  hi LineNr ctermfg=244 ctermbg=NONE cterm=NONE
  hi FoldColumn ctermfg=244 ctermbg=NONE cterm=NONE
  hi CursorLine ctermfg=NONE ctermbg=252 cterm=NONE
  hi CursorColumn ctermfg=NONE ctermbg=252 cterm=NONE
  hi CursorLineNr ctermfg=NONE ctermbg=252 cterm=NONE
  hi QuickFixLine ctermfg=NONE ctermbg=252 cterm=NONE
  hi SignColumn ctermfg=NONE ctermbg=NONE cterm=NONE
  hi Underlined ctermfg=30 ctermbg=NONE cterm=underline
  hi Error ctermfg=160 ctermbg=254 cterm=NONE
  hi ErrorMsg ctermfg=160 ctermbg=254 cterm=NONE
  hi WarningMsg ctermfg=55 ctermbg=254 cterm=NONE
  hi MoreMsg ctermfg=16 ctermbg=254 cterm=NONE
  hi ModeMsg ctermfg=16 ctermbg=254 cterm=NONE
  hi Question ctermfg=29 ctermbg=NONE cterm=NONE
  hi Todo ctermfg=254 ctermbg=244 cterm=NONE
  hi MatchParen ctermfg=254 ctermbg=62 cterm=NONE
  hi Search ctermfg=254 ctermbg=60 cterm=NONE
  hi IncSearch ctermfg=254 ctermbg=29 cterm=NONE
  hi WildMenu ctermfg=254 ctermbg=60 cterm=NONE
  hi ColorColumn ctermfg=NONE ctermbg=252 cterm=NONE
  hi Cursor ctermfg=254 ctermbg=16 cterm=NONE
  hi lCursor ctermfg=16 ctermbg=161 cterm=NONE
  hi DiffAdd ctermfg=NONE ctermbg=194 cterm=NONE
  hi DiffChange ctermfg=NONE ctermbg=230 cterm=NONE
  hi DiffText ctermfg=NONE ctermbg=222 cterm=NONE
  hi DiffDelete ctermfg=244 ctermbg=224 cterm=NONE
  hi SpellBad ctermfg=124 ctermbg=NONE cterm=underline
  hi SpellCap ctermfg=29 ctermbg=NONE cterm=underline
  hi SpellLocal ctermfg=62 ctermbg=NONE cterm=underline
  hi SpellRare ctermfg=127 ctermbg=NONE cterm=underline
  hi Identifier ctermfg=30 ctermbg=NONE cterm=NONE
  hi Statement ctermfg=124 ctermbg=NONE cterm=NONE
  hi Constant ctermfg=161 ctermbg=NONE cterm=NONE
  hi String ctermfg=29 ctermbg=NONE cterm=NONE
  hi PreProc ctermfg=55 ctermbg=NONE cterm=NONE
  hi Special ctermfg=62 ctermbg=NONE cterm=NONE
  hi Tag ctermfg=55 ctermbg=NONE cterm=NONE
  hi Delimiter ctermfg=161 ctermbg=NONE cterm=NONE
  hi Type ctermfg=127 ctermbg=NONE cterm=NONE
  hi Operator ctermfg=55 ctermbg=NONE cterm=NONE
  hi Directory ctermfg=30 ctermbg=NONE cterm=bold
  hi Comment ctermfg=244 ctermbg=NONE cterm=NONE
  hi Conceal ctermfg=244 ctermbg=NONE cterm=NONE
  hi Ignore ctermfg=NONE ctermbg=NONE cterm=NONE
  hi Title ctermfg=124 ctermbg=NONE cterm=bold
  hi! link colortemplateKey Statement
  hi! link colortemplateAttr String
  hi! link vimNotation Type
  hi! link vimFuncSID PreProc
  hi! link vimHiTerm Identifier
  hi! link helpNotVi Comment
  hi! link helpExample PreProc
  hi! link gdscriptBlockStart Statement
  hi! link gdscriptFunctionName Function
  hi! link gitCommitSummary Title
  hi! link cocErrorSign Type
  unlet s:t_Co
  finish
endif

" Background: light
" Color: comment    #808080 244
" Color: constant   #d7005f 161
" Color: string     #2e8b57 29
" Color: identifier #008787 30
" Color: statement  #a52a2a 124
" Color: preproc    #6a0dad 55
" Color: type       #af00af 127
" Color: special    #6a5acd 62
" Color: fg0        #000000 16
" Color: bg0        #e0e3e3 254
" Color: bg1        #bcbcbc 251
" Color: folded     #d0d0d0 252
" Color: visual     #ffffd7 230
" Color: error      #d70000 160
" Color: wildmenu   #5f5f87 60
" Color: diffadd    #c9f9c9 194
" Color: diffchange #f9f9c9 230
" Color: difftext   #f9d999 222
" Color: diffdelete #f9c9c9 224
" Term colors: bg0     statement string preproc identifier type special bg1
" Term colors: comment statement string preproc identifier type special fg0
" vim: et ts=2 sw=2
